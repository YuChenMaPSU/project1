...
# ╔═╡ fa826b98-58b9-11ed-24aa-35ff8e54803c
begin
	using Downloads
...
# ╔═╡ f35c40ce-cb10-4303-bfd4-e70e8e406e03
begin
    path = joinpath(pwd(),"data")
    mkpath(path)
    filename = "legacy_data.csv"
    datapath=joinpath(path,"legacy_data.csv")
    if !isfile(datapath) || filesize(datapath)==0
        Downloads.download("https://github.com/leerosenthalj/CLSI/raw/master/legacy_tables/legacy_data.csv",datapath)
        fresh_data = true
    else
        fresh_data = false
    end
...
# ╔═╡ a69d9b81-60ac-48fa-9249-5d2ae55d7542
begin
    fresh_data
    df_raw = CSV.read(datapath,DataFrame)
end

# ╔═╡ ae03f8ed-0678-4ae1-ac8b-bf00cbe85359
begin
		select!(df_raw,["name","jd","mnvel","errvel","sval","tel"])
	rename!(df_raw,["name"=>"Name","jd"=>"JD","mnvel"=>"RV","errvel"=>"e_RV","sval"=>"SVal","tel"=>"Inst"])
end
